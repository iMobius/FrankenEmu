...
void emu(double *xstar, double *ystar, int *outtype) {
    ...
    for(i=0; i<p; i++) {
        if((xstar[i] < xmin[i]) || (xstar[i] > xmin[i]+xrange[i])) {
            ...
            switch(i) {
                case 0:
                    printf("omega_b must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
                case 1:
                    printf("omega_m must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
                case 2:
                    printf("n_s must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
				case 3:
					         printf("h must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
                case 4:
                    printf("w must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
                case 5:
                    printf("sigma_8 must be between %f and %f. value=%f\n", xmin[i], xmin[i]+xrange[i],xstar[i]);
                    break;
            }
            exit(1);
        }
